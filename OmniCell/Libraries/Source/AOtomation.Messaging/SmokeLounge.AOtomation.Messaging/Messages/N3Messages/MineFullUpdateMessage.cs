// --------------------------------------------------------------------------------------------------------------------
// <copyright file="MineFullUpdateMessage.cs" company="OmniCell">
...
// </copyright>
// <summary>
//   Defines the MineFullUpdateMessage type.
// </summary>
// --------------------------------------------------------------------------------------------------------------------
...
namespace SmokeLounge.AOtomation.Messaging.Messages.N3Messages
{
    ...
    /// <summary>
    /// A mine: everything <see cref="TrapItemFullUpdateMessage"/> carries, and
    /// a version and an int32 after it.
    /// </summary>
    /// <remarks>
    /// Extracted-client MineFullUpdateIIR_t has vtable 0x10167C1C, reader
    /// 0x100A0DA4, writer 0x100A0DF1 and dispatcher 0x100A0EA5. The reader
    /// calls TrapItemFullUpdate's reader at 0x100A28CA for the whole of that
    /// message, then takes an int32 version checked against the static at
    /// 0x101C2088 and one int32; the writer emits the static and the same
    /// int32.
    ///
    /// No capture contains one.
    /// </remarks>
    [AoContract((int)N3MessageType.MineFullUpdate)]
    public class MineFullUpdateMessage : N3Message
    {
        public MineFullUpdateMessage()
        {
            this.N3MessageType = N3MessageType.MineFullUpdate;
        }
        ...
        /// <summary>
        /// Checked against the static at 0x101C2088.
        /// </summary>
        /// <remarks>
        /// The writer at 0x100A0DF1 emits the static rather than a field.
        /// </remarks>
        [AoMember(18)]
        public int MineVersion { get; set; }

        /// <summary>
        /// An int32 with no name.
        /// </summary>
        /// <remarks>
        /// Read into the message at + 0x8C at 0x100A0DE0 and written back out.
        /// Nothing else in the class touches it: the dispatcher at 0x100A0EA5
        /// only checks the target exists, and the two copy-backs the class
        /// inherits are TrapItemFullUpdate's. No capture contains one.
        /// </remarks>
        [AoMember(19)]
